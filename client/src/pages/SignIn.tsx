import { useState } from 'react';
import { Link } from 'react-router-dom';
import axios from 'axios';
import LoadingButton from '@mui/lab/LoadingButton';
import PersonAddIcon from '@mui/icons-material/PersonAddTwoTone';
import { useSnackbar } from 'notistack';
import { useNavigate } from 'react-router-dom';
import { useDispatch, useSelector } from 'react-redux';
import { signInStart, signInSuccess, signInFailure } from '../redux/user/userSlice';

import styles from "./styles.module.css";

const GoogleSVGIcon = () => (
    <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="24" height="24" viewBox="0 0 48 48">
        <path fill="#FFC107" d="M43.611,20.083H42V20H24v8h11.303c-1.649,4.657-6.08,8-11.303,8c-6.627,0-12-5.373-12-12c0-6.627,5.373-12,12-12c3.059,0,5.842,1.154,7.961,3.039l5.657-5.657C34.046,6.053,29.268,4,24,4C12.955,4,4,12.955,4,24c0,11.045,8.955,20,20,20c11.045,0,20-8.955,20-20C44,22.659,43.862,21.35,43.611,20.083z"></path><path fill="#FF3D00" d="M6.306,14.691l6.571,4.819C14.655,15.108,18.961,12,24,12c3.059,0,5.842,1.154,7.961,3.039l5.657-5.657C34.046,6.053,29.268,4,24,4C16.318,4,9.656,8.337,6.306,14.691z"></path><path fill="#4CAF50" d="M24,44c5.166,0,9.86-1.977,13.409-5.192l-6.19-5.238C29.211,35.091,26.715,36,24,36c-5.202,0-9.619-3.317-11.283-7.946l-6.522,5.025C9.505,39.556,16.227,44,24,44z"></path><path fill="#1976D2" d="M43.611,20.083H42V20H24v8h11.303c-0.792,2.237-2.231,4.166-4.087,5.571c0.001-0.001,0.002-0.001,0.003-0.002l6.19,5.238C36.971,39.205,44,34,44,24C44,22.659,43.862,21.35,43.611,20.083z"></path>
    </svg>)


function SignIn() {
    const [email, setEmail] = useState("");
    const [password, setPassword] = useState("");
    const { loading, error } = useSelector((state: any) => state.user);

    const { enqueueSnackbar } = useSnackbar();
    const navigate = useNavigate();
    const dispatch = useDispatch();

    const onChangeHandler = (event: React.ChangeEvent<HTMLInputElement>) => {
        const { id, value } = event.target;
        switch (id) {
            case "email":
                setEmail(value);
                break;
            case "password":
                setPassword(value);
                break;
            default:
                break;
        }
    }

    const saveSignInDetails = (event: React.FormEvent<HTMLFormElement>) => {
        event.preventDefault();

        dispatch(signInStart())
        const data = {
            email: email,
            password: password
        }

        axios.post("/api/v1/users/signin", data)
            .then(_response => {
                enqueueSnackbar('You have been successfully logged in!', { variant: "success", autoHideDuration: 3000, anchorOrigin: { vertical: 'top', horizontal: 'center' } });
                dispatch(signInSuccess(_response.data))

                // clear the form                
                setEmail("");
                setPassword("");

                // redirect to home page
                navigate("/");
            })
            .catch(err => {
                enqueueSnackbar(`${err?.response?.data?.message}`, { variant: "error", autoHideDuration: 3000, anchorOrigin: { vertical: 'top', horizontal: 'center' } });
                dispatch(signInFailure(err?.response?.data?.message))
            })
    }

    return (
        <div className={styles.signup_container}>
            <h1>Sign In</h1>
            <form onSubmit={saveSignInDetails}>
                <label htmlFor='email'>Email</label>
                <input type='email' id='email' placeholder="Email" value={email} onChange={onChangeHandler} required />
                <label htmlFor='password'>Password</label>
                <input type='password' id='password' placeholder="Password" value={password} onChange={onChangeHandler} required />
                <LoadingButton
                    loading={loading}
                    loadingPosition="start"
                    startIcon={<PersonAddIcon />}
                    variant="contained"
                    type='submit'
                >
                    Sign In
                </LoadingButton>
                <LoadingButton
                    loading={false}
                    loadingPosition="start"
                    startIcon={<GoogleSVGIcon />}
                    variant="contained"
                    style={{ backgroundColor: "maroon", color: "white" }}
                >
                    Login with Google
                </LoadingButton>
            </form>
            <div style={{ marginTop: "10px" }}>
                Don't have account? <span style={{ color: "darkblue", fontWeight: "bold" }}><Link to={"/sign-up"}>Sign Up</Link></span>
            </div>
        </div>
    )
}

export default SignIn
